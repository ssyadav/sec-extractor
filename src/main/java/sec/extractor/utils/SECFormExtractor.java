...
public class SECFormExtractor {
	...
	public static List<SECItemSection> extractForm10K(String xhtml) {
		...
		List<SECItemSection> listOfItemSections = extractFormContent(xhtml, sectionTitlesFileName);
		
		if(listOfItemSections.size() == 0) {
			listOfItemSections = extractForm10UsingDiv(xhtml);
		}
	 ...
	 */
	private static List<SECItemSection> extractFormContent(String xhtml, String sectionTitlesFileName) {
	...
	public static List<SECItemSection> extractForm10UsingDiv(String xhtml) {
		...
		try {
			...
			for(Element elem : mainBodyElements){
				titles.addAll(extractTitles(elem.children(), SECConstants.FORM_10K_FILE_NAME));
