 ...
 *
 */
public class SECFormExtractorFailureHelper {


	private static final Logger LOGGER = LoggerFactory.getLogger(SECFormExtractorFailureHelper.class);
	...
	public static List<SECItemSection> extractForm10K(String xhtml) {
		...
		try {
			...
			if (null != doc) {
				...
				//Elements bodyElem = doc.getElementsByTag(SECConstants.STRING_BODY).select("*");
				Elements bodyElem = doc.body().children();
				
				for (Iterator<Element> iterator = bodyElem.iterator(); iterator.hasNext();) {
					Element element2 = iterator.next();
					System.out.println(element2);
					
					
				}
				//Elements mainElements = doc.body().select("*");
				System.out.println(bodyElem.size());
				for (int elementIndex = 0; elementIndex < bodyElem.size(); elementIndex++) {
				    Elements bodyElemen = bodyElem.get(elementIndex).children();
				    //for (Element elf : bodyElemen) {
				    	//Elements bodyElements = elf.children();
				    	System.out.println(bodyElemen.toString());
					    listOfItemSections = extractForm10KFromElements(bodyElemen, titles);
...
//				    }
	...
	public static List<SECItemSection> extractForm10KFromElements(Elements bodyElements, List<String> titles) {
		List<SECItemSection> listOfItemSections = new ArrayList<SECItemSection>();
...
/*
				...
				for (int titleIndex = 0; titleIndex < titles.size(); titleIndex++) {
					...
					for (int elementIndex = 0; elementIndex < bodyElements.size(); elementIndex++) {
						Element currentElement = bodyElements.get(elementIndex);
						String currentNodeText = currentElement.ownText();
						boolean canBreak = false;
						boolean isTitleNode = false;
						if (null != currentNodeText && !currentNodeText.equals(SECConstants.EMPTY_STRING) 
								&& !currentNodeText.equals(SECConstants.STRING_WITH_SINGLE_SPACE)) {
							...
							if (!searchTitleFound.isEmpty()) {
								sectionContent = new StringBuilder();
								String titlePlusItemNumber[] = {"1","2"};//property.getProperty(searchTitleFound).split(SECConstants.STRING_COLON_HYFHAN);
								itemSection.setSectionTitle(""+elementIndex);
								itemSection.setItemNumber(titlePlusItemNumber[1]);
								isTitleNode = true;
								LOGGER.info(" ***** SEARCH TITLE FOUND ***** " + currentNodeText);
							} else {
