 ...
 *
 */
public class Item10KProcessor {

	private static final Logger LOGGER = LoggerFactory.getLogger(Item10KProcessor.class);
	private static String EMPTY_STRING = "";
	
	static String localInput = "test/input/10k-25/";
	static String localOutput = "test/output/10k-25/";
	...
	private static List<SECItemSection> fetchSecItemSection(String xhtml) {
		...
		List<SECItemSection> listOfItemSections = SECFormExtractor.extractForm10K(xhtml);
		...
		if(!listOfItemSections.isEmpty()) {
			...
			for (SECItemSection item : listOfItemSections) {
				...
				try {
					...
					String body = item.getItemNumber() + " : " +  title + "<br> " + item.getSectionContent().toString();
